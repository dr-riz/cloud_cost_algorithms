...
public class GreedOnInvestment extends Algorithm {
	...
	GreedOnInvestment(double ar, TransactionClass tc, int useCase) {
		super(ar, tc, useCase);
		m_name = "greedy";
		// TODO Auto-generated constructor stub
	}

	GreedOnInvestment(Vector<TransactionClass> tcs, Vector<Partition> dps, int useCase) {		
	...
	public void calcCostOf_new_conf() {
		...
		printStep("greedOnInvestment", m_scenario, m_confp, costp, m_numMinima);
	}
	...
	public boolean currentConfigStableButNewUnstable() {
		...
		if (m_confp.unstable() && !m_conf.unstable()) {
			...
			m_minimum = m_conf;
			m_numMinima++; 
			printStep("greedOnInvestment", m_scenario, m_conf, m_minimum.cost(), m_numMinima);
